...
char * names[100] = {NULL}; //may not want global array, but let's try it.

int addNames(char * name1, char * name2) {
    int i = 0;
    ...
    names[i] = strdup(name1);
    names[i+1] = strdup(name2);
    return 1;
}
...
void readNames(struct pollfd * pfds, char *player1, char *player2) {
    ...
    for (;;) {
	    ...
	    if (pfds[0].revents && ready1 == 0) {
            ...
            buf[bytes] = '\0';
            char *check;
            if (strncmp(buf, "P|", 2) == 0 && (check = strstr(buf, "||")) != NULL) {
                int name_length = check - (buf + 2);
	    ...
	    if (pfds[1].revents && ready2 == 0) {
            ...
            buf2[bytes2] = '\0';
            char *check2;
            if (strncmp(buf2, "P|", 2) == 0 && (check2 = strstr(buf2, "||")) != NULL) {
                int name_length = check2 - (buf2 + 2);
...
void processMoves(struct pollfd * pfds, char *player1, char *player2) {
    ...
    printf("The game is ready. Please enter a command\n");
    ...
    for (;;) {
        ...
        if (pfds[0].revents && !move1_ready) {
            bytes = read(pfds[0].fd, buf, BUFLEN-1);
                ...
                buf[bytes] = '\0';
                char *check;
                if (strncmp(buf, "M|", 2) == 0 && (check = strstr(buf, "||")) != NULL) {
                    int move_length = check - (buf + 2);
        ...
        if (pfds[1].revents && !move2_ready) {
            bytes2 = read(pfds[1].fd, buf2, BUFLEN);
                ...
                buf2[bytes2] = '\0';
                char *check2;
                if (strncmp(buf2, "M|", 2) == 0 && (check2 = strstr(buf2, "||")) != NULL) {
                    int move_length = check2 - (buf2 + 2);
...
int main(int argc, char **argv)
{
    ...
    printf("Server is running (port number is -> %s) \n", service);
    
    for(;;) {
        int player = accept(listen, NULL, NULL);
        if (player < 0) {
            perror("accept error occurred");
            continue;
        }
        printf("Player connected!\n");

        game_lobby[count] = player;
        count = count + 1;
        ...
        if (count == 2) {
            ...
            if (pid == 0) {
                ...
                pfds[0].fd = game_lobby[0];
                pfds[1].fd = game_lobby[1];
                pfds[0].events = POLLIN;
                pfds[1].events = POLLIN;

                char player1[NAMELEN], player2[NAMELEN];
                readNames(pfds, player1, player2);
                // We tried to handle a duplicate player, but failed.
                /*int i = 0;
                printf("iteration1\n");
                while(names[i] != NULL){
                    printf("name[%d]: %s\n", i , names[i]);
